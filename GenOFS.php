...
# copy upload file to data folder
if ($fra != null) {		// if a file has been specified , copy it in the directory $til (Data_files folder)
	copy($fra, $til);
	if ($debug) { $log->lwrite(' * Copied ' . $fra . ' to ' . $til); }
	
} else {
  if ($debug) { $log->lwrite(' * Nofile Error'); }
	header("Location:AddDataOFS.php?error=nofile"); 
	return;
}

if ($debug) { $log->lwrite(' * Reading text from: ' . $wwwDataDirectory . $fileName); }
$text = readTxtFile($wwwDataDirectory . $fileName );	// read file
if ($text == null) {
  if ($debug) { $log->lwrite(' * Fileformat error'); }
...
if ($debug) { $log->lwrite(' * Number of columns: ' . $nrCol); }
...
if ($debug) { $log->lwrite(' * Nr of lines after check: ' . count($linesOK)); }

$sql_drop_dump="drop table if exists ofs_dump";
if ($debug) { $log->lwrite(' * SQL drop dump: ' . $sql_drop_dump); }
...
#echo $year;
  ...
  if ($debug) { $log->lwrite(' * Missing year from input error'); }
	header("Location:AddDataOFS.php?error=year");
}
if ($debug) { $log->lwrite(' * Year from input: ' . $year); }
...
if ($debug) { $log->lwrite(' * Creating ofs_dump table: ' . $sql_create_dump); }
pg_query($sql_create_dump);

//copy the file in the ofs_dump table
pg_copy_from($dbh, "ofs_dump", $linesOK, ";");
if ($debug) { $log->lwrite(' * Copied input to dump-table'); }

$sql_create_ofs = "create table ofs_".$year." as (select * from ofs)"; 
if ($debug) { $log->lwrite(' * Create year-specific ofs table: ' . $sql_create_ofs); }
...
if ($not_empty==$num_field){ 
	...
	if ($debug) { $log->lwrite(' * Add column sql: ' . $sql_add_column); }
	...
	if ($debug) { $log->lwrite(' * Percent farmer sql: ' . $sql_percent_farmer0); }
	...
	if ($debug) { $log->lwrite(' * Percent grazing: ' . $sql_percent_surface_grazing0); }
	...
	for ($m=0;$m<$num_field_plz;$m++){
		...
		where b.num_ofs=a.num_ofs)";
		if ($debug) { $log->lwrite(' * SQL join ofs plz for m: ' . $m . ' -- ' . $join_ofs_plz); }
		pg_query($join_ofs_plz);
	}
	$sql_drop_table="drop table ofs_".$year;
	if ($debug) { $log->lwrite(' * SQL drop table: ' . $sql_drop_table); }
	pg_query($sql_drop_table);
	if ($debug) { $log->lwrite(' * Compute IndexSocioEcPLZ for year: ' . $year); }
